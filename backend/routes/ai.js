...
router.post('/write', requireAnyAuth, async (req, res) => {
  ...
  if (m === 'refine') {
    ...
    userPrompt = isVillageDiary
      ? `다음 마을일기 한 줄의 톤만 다듬어라. **원문의 사실·사건·고유명사를 그대로 유지하고, 새로운 내용·사물·사건을 추가하지 마라.** 위 기준(한 문장·40자 내외·마침표로 끝)에 맞게 행정 어투만 걷어내 더 서정적으로. 결과는 그 한 문장만 출력하라.

[원문]
${src}`
      : `다음 글의 톤만 다듬어라. **원문의 사실·사건·고유명사를 유지한 채 톤만 다듬고, 새로운 내용을 추가하지 마라.** 행정 어투를 걷어내고 장면과 정서가 흐르는 서정적 산문으로. 결과는 본문만 출력하라(제목·머리말·설명 금지).
...
${src}`;
    ...
    userPrompt = isVillageDiary
      ? `아래 주제/키워드의 내용을 중심으로, 위 기준(한 문장·40자 내외·마침표로 끝나는 마을일기 한 줄)에 맞는 일기 한 줄을 써라.
**아래 주제/키워드의 내용이 문장의 중심이 되어야 한다. 키워드에 없는 사건·사물을 지어내지 마라.** 프로그램 맥락은 배경 참고일 뿐, 키워드를 대체하지 않는다. 결과는 그 한 문장만 출력하라.

주제/키워드: ${subject} — 이 내용으로 쓸 것`
      : `아래 주제/키워드의 내용을 중심으로, 위 서정의 기준에 맞는 글을 써라.
**아래 주제/키워드의 내용이 글의 중심이 되어야 한다. 키워드에 없는 사건·사물을 지어내지 마라.** 결과는 본문만 출력하라(제목·머리말·설명 금지). 2~4개 단락 분량.
