...
//   · 대시보드: 누적 참여(연인원, Σ세션 attendance) / Σ정원 222 + 프로그램별 회차 진척 바.
...
//   레이아웃(총괄 카드 그리드 + 프로그램 카드 골격)은 즉시 그려지고 데이터만 비워둔다.
//   색·반짝임은 businessAdmin.css 의 --kd-* 토큰/ink-with-alpha 만 사용(임의 컬러 금지).
//   총괄 카드는 .kd-admin-totals 반응형 그리드를 재사용 → 데스크톱·모바일 공통.
const DashSkelBar: React.FC<{ w?: string; h?: number }> = ({ w = '100%', h = 14 }) => (
  <span className="kd-skel-bar" style={{ width: w, height: h }} aria-hidden="true" />
);

const DashboardSkeleton: React.FC = () => (
  <div
    className="kd-admin-dashboard kd-skel"
    role="status"
    aria-busy="true"
    aria-live="polite"
    aria-label="대시보드를 불러오는 중"
  >
    <span className="kd-skel-sronly">대시보드를 불러오는 중…</span>
    <div className="kd-admin-totals" aria-hidden="true">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="kd-admin-total-card">
          <DashSkelBar w="52%" h={12} />
          <DashSkelBar w="78%" h={20} />
        </div>
      ))}
    </div>
    <div className="kd-dash-programs" aria-hidden="true">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="kd-dash-program kd-skel-program">
          <div className="kd-dash-program-head">
            <DashSkelBar w="46%" h={16} />
            <DashSkelBar w="84px" h={28} />
          </div>
          <div className="kd-dash-bar-row">
            <DashSkelBar w="100%" h={10} />
          </div>
          <DashSkelBar w="38%" h={12} />
        </div>
      ))}
    </div>
  </div>
            ...
            <div key={p.key} className="kd-dash-program">
              ...
              {isOpen && (
                <div className="kd-dash-sessions">
                  ...
                  {sessionsLoading ? (
                    <table
                      className="kd-admin-table kd-dash-session-table kd-skel-table"
                      role="status"
                      aria-busy="true"
                      aria-live="polite"
                      aria-label="회차 목록 불러오는 중"
                    >
                      <tbody>
                        {[0, 1, 2].map((r) => (
                          <tr key={r}>
                            ...
                            <td className="kd-admin-td-name"><span className="kd-skel-bar" style={{ width: '80%' }} /></td>
                            <td className="kd-admin-td-num"><span className="kd-skel-bar" style={{ width: '45%' }} /></td>
                            <td className="kd-admin-td-name"><span className="kd-skel-bar" style={{ width: '55%' }} /></td>
                            <td className="kd-admin-td-name"><span className="kd-skel-bar" style={{ width: '50%' }} /></td>
                          </tr>
                        ))}
                      </tbody>
