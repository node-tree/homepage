...
const Filed: React.FC<FiledProps> = ({ onPostsLoaded }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          const postsResponse = await filedAPI.getAllPosts({ forceRefresh: attempt > 0 });
          if (!isMounted) return;
          ...
          if (postsResponse.success) {
            ...
            if (attempt < maxRetries) {
              console.log(`Filed posts: 빈 응답, ${attempt + 1}/${maxRetries} 재시도 중...`);
          ...
          if (attempt < maxRetries && isMounted) {
            console.log(`Filed posts: 에러 발생, ${attempt + 1}/${maxRetries} 재시도 중...`);
            await new Promise(resolve => setTimeout(resolve, retryDelay));
            continue;
          }
          if (isMounted) {
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      await filedAPI.reorderPosts(orders);
